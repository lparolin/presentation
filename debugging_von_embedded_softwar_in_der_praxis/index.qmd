...
Die Verwendung des Begriffs ‚Bug‘ für technische Fehler lässt sich bis ins 19. Jahrhundert zurückverfolgen.
...
**Beobachtbarkeit des Defekts**

- Ein zentrales Thema dieser Vorlesung ist die _Beobachtbarkeit_ von Defekten.
- Effektives Debugging setzt voraus, dass ein Defekt **wiederholt stimuliert** und sichtbar gemacht werden kann.
- In eingebetteten Systemen ist dies jedoch häufig schwierig oder gar unmöglich
  - Fast immer sind mehrere Einflussfaktoren (z. B. elektrische Störungen, zeitliche Randbedingungen, Umgebungsbedingungen) nicht unter unserer Kontrolle und oft auch nicht zuverlässig messbar.
- Ohne Beobachtbarkeit gibt es keine Reproduzierbarkeit, und ohne Reproduzierbarkeit ist Software‑Debugging unmöglich.

::: {.callout-note appearance="simple" .fade-left}
Um Software zu debuggen, ist es höchst erforderlich **den Fehler immer wieder erzwingen!**[^agans]
:::
...
- Die Entwicklung erfordert **Zusammenarbeit zwischen verschiedenen Teams und Disziplinen**
...
### Anforderungen, Tests und Teamkommunikation
...
Wir wollen ein Adaptive Geschwindigkeitregelung (ACC) entwickeln und die vordere Kamera ist ein Kernsensor.

::: {.fragment}

- **Anforderung P.1:** ACC muss aktivierbar sein, wenn alle Sensoren einen nominalen Zustand melden (keine Degradation).
- **Anforderung P.2:** ACC muss sich sicher deaktivieren, wenn eine, oder mehrere Sensoren eine Degradation melden.
...
- **Anforderung P.1:** ACC muss aktivierbar sein, wenn alle Sensoren einen nominalen Zustand melden (keine Degradation).
- [**Anforderung P.2:** ACC muss sich sicher deaktivieren, wenn eine, oder mehrere Sensoren eine Degradation melden.]{style="color: dimgray;"}
- ...
...
| ------------ | ----------------------------------------- | -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ---------- | ---------- |
| **P.1 - 01** | _Ideales Wetter_: Tag, Sonne, kaum Wolken | ACC ist aktivierbar und läuft ohne Unterbrechungen | Fahrzeug fährt korrekt bei freier Fahrt, bleibt jedoch stehen, sobald eine Stausituation entsteht. → **ACC nicht mehr aktivierbar!** | 1.1        | C          |
...
| Test ID      | Vorbedingungen                          | Soll-Verhalten                                     | Ist-Verhalten                                                                                                                        | SW-Version | HW-Version |
| ------------ | --------------------------------------- | -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ---------- | ---------- |
| **P.1 - 01** | Ideales Wetter: Tag, Sonne, kaum Wolken | ACC ist aktivierbar und läuft ohne Unterbrechungen | Fahrzeug fährt korrekt bei freier Fahrt, bleibt jedoch stehen, sobald eine Stausituation entsteht. → **ACC nicht mehr aktivierbar!** | 1.1        | C          |
...
## Präventivemaßnahmen sind in der Praxis unverzichtbar {auto-animate=true}

### Ziel ist es, die Notwendigkeit von Debugging zu reduzieren
...
- Moderne Mikrocontroller besitzen fast immer **dedizierte Debugging-Hardware**
  - Das ermöglicht Zugriff auf Register, Speicher und CPU-Kontrolle
- Typische Werkzeuge: JTAG (Joint Test Action Group)^[IEEE 1149.1 – Standard Test Access Port and Boundary-Scan Architecture (IEEE, 2013)] + OpenOCD (Open On‑Chip Debugger) + GDB (GNU Debugger)

:::{.incremental}

- Debugging erfordert die Verwendung von Debug‑Flags **Debug-Flags**
  - Das kann Performance und Ablauf gegenüber Optimierungs‑Flags wie `-O2` verändern
- **Zeitliche Probleme sind dann nicht immer sichtbar**, da die reale Code-Performance abweicht
